...
const SeaLevelChart = () => {
    ...
    useEffect(() => { // useEffect för att utföra datahämtning vid första rendering
        const fetchData = async () => { // Hämtar och bearbetar data från en JSON-fil genom en asynkron funktion
            try {
                ...
                const response = await fetch('Dataset4_Sea Level .json');
                const result = await response.json();
                console.log('API Response (First 5 items):', result.slice(0, 5));
                // Formar datan så den passar för Recharts
                const formattedData = result.map((item) => ({
                    // Visar år och vattennivå
                    year: item.Time.split('-')[0],
                    sea_level: parseFloat(item.GMSL),
                }));
    ...
    return (
        <ResponsiveContainer width="100%" height={400}>
            <LineChart data={data}>
                <Line type="monotone" dataKey="sea_level" stroke="#82ca9d" />
